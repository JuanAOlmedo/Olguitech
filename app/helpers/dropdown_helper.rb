...
module DropdownHelper
    ...
    class Dropdown
        ...
        delegate :link_to, :button_to, :tag, :image_tag, :safe_join, :raw, to: :view

        def trigger
            tag.div class: 'dropdown-trigger' do
                tag.button button, class: 'btn', data: { action: 'click->dropdown#display click@window->dropdown#stopDisplayingWhenOutside' },
                                   aria: { haspopup: 'true', controls: 'dropdown-menu' }
            end
        end

        def icon
            icon = raw '<svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-world" width="16" height="16" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"> <path stroke="none" d="M0 0h24v24H0z" fill="none"/> <circle cx="12" cy="12" r="9" /> <line x1="3.6" y1="9" x2="20.4" y2="9" /> <line x1="3.6" y1="15" x2="20.4" y2="15" /> <path d="M11.5 3a17 17 0 0 0 0 18" /> <path d="M12.5 3a17 17 0 0 1 0 18" /> </svg>'

            tag.span icon, class: 'dropdown-icon localized'
        end

        def button
            chevron = raw '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"> <path stroke="none" d="M0 0h24v24H0z" fill="none"/> <polyline points="6 12 12 18 18 12" /> </svg>'

            chevron = tag.span chevron, class: 'dropdown-icon'

            @icon ? safe_join([icon, name, chevron]) : safe_join([name, chevron])
        end
