...
export default class extends Controller {
    static targets = ["select", "solution", "project", "product"];
    id = -1;
    ...
    changeIds(node) {
        node.children[0].innerHtml = this.replaceZeroById(
            node.children[0].innerHTML,
        );
        node.children[1].attributes.name.value = this.replaceZeroById(
            node.children[1].attributes.name.value,
        );
        node.children[1].attributes.id.value = this.replaceZeroById(
            node.children[1].attributes.id.value,
        );
    ...
    frametagChange(event) {
        ...
        if (form) {
            ...
            form.children[0].innerHTML = this.replaceZeroById(
    ...
    changeDisplayedArticles(select) {
        ...
        console.log(this.solutionTargets);
        if (this.solutionTargets[id].name != "") {
            select.dataset.name = this.solutionTargets[id].name;
        }

        this.remove(this.solutionTargets[id]);
        this.remove(this.projectTargets[id]);
        this.remove(this.productTargets[id]);

        switch (select.selectedOptions[0].value) {
            case "Solution":
                this.add(this.solutionTargets[id], select.dataset.name);
                break;
            case "Project":
